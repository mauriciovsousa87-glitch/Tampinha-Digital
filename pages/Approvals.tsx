  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <header>
        ...
        <p className="text-zinc-400 font-bold uppercase text-[10px] tracking-widest mt-1 italic">Gestão de solicitações de resgate.</p>
      </header>

      <div className="flex gap-2 bg-white p-2 rounded-2xl w-fit border border-zinc-200 shadow-sm">
        <button 
          onClick={() => setActiveTab('PENDING')}
          className={`px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${activeTab === 'PENDING' ? 'bg-yellow-500 text-black shadow-lg shadow-yellow-500/10' : 'text-zinc-400 hover:text-zinc-900'}`}
        >
          Pendentes ({db.orders.filter((o: any) => o.status === OrderStatus.PENDING).length})
        </button>
        <button 
          onClick={() => setActiveTab('HISTORY')}
          className={`px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${activeTab === 'HISTORY' ? 'bg-zinc-900 text-white shadow-lg shadow-zinc-900/10' : 'text-zinc-400 hover:text-zinc-900'}`}
        >
          Histórico Total
        </button>
      </div>
      ...
      <div className="grid gap-6">
        {filteredOrders.length === 0 ? (
          <div className="bg-white p-20 rounded-[3rem] border border-dashed border-zinc-200 text-center flex flex-col items-center shadow-sm">
             ...
             <p className="text-zinc-400 font-black uppercase text-xs tracking-widest italic">Nada para mostrar aqui no momento.</p>
          </div>
        ) : (
